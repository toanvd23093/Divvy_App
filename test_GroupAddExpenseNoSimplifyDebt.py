...
# --- Test Points ---
def test_addExpenses_3person(): #1
...
def test_WhoPaidNotValidUser(): #8
    # Create users and group
    toan, bich, eugene, pho = setup_users_and_group()
    bibibi = Group("23GrampianWay", [toan,bich,], False)
    bibibi2 = Group("25GrampianWay", [toan,bich,], False)

    # Add expenses
    with pytest.raises(ValueError,match="User not recognized"):
        bibibi.addExpenseSinglePayer([bich,toan],90,bibibi2,"dinner")

def test_InvalidExpenses(): #9
    # Create users and group
    toan, bich, eugene, pho = setup_users_and_group()
    bibibi = Group("23GrampianWay", [toan,bich,], False)
    ...
    with pytest.raises(ValueError,match="Total expense is not valid. Must be an integer or a float number"):
        bibibi.addExpenseSinglePayer([bich,toan],"a",toan,"dinner")

def test_NegativeExpenses(): #10
    # Create users and group
    toan, bich, eugene, pho = setup_users_and_group()
    bibibi = Group("23GrampianWay", [toan,bich,], False)
    ...
    with pytest.raises(ValueError,match="Total expense must be positive"):
        bibibi.addExpenseSinglePayer([bich,toan],-100,toan,"dinner")

def test_printOutResults(): #11
    # Create users and group
    toan, bich, eugene, pho = setup_users_and_group()
    bibibi = Group("23GrampianWay", [toan,bich,], True)

    bibibi.addExpenseSinglePayer([bich,toan],100,toan,"dinner")
    bibibi.splitExpensesCalculation()
    
    tmpDict = {}
    for lender in bibibi.ResultsDB.keys():
        tmpDict.update({f"{lender.userFirstName} {lender.userLastName}":{}})
        for borrower in bibibi.ResultsDB[lender].keys():
            tmpDict[f"{lender.userFirstName} {lender.userLastName}"].update({f"{borrower.userFirstName} {borrower.userLastName}":f"{bibibi.ResultsDB[lender][borrower]:.2f}"})
 
    actResult =  pd.DataFrame(tmpDict).T
    
    expResult = bibibi.printOutResult()
    
    assert actResult.equals(expResult) == True

def test_calculateExpenses7(): #12
    # Create users and group
    toan, bich, eugene, pho = setup_users_and_group()
    bibibi = Group("23GrampianWay", [toan,bich], False)

    # Add expenses
    bibibi.addExpenseSinglePayer([bich,toan],90,toan,"dinner")
    bibibi.addExpenseSinglePayer([bich,toan],40,bich,"dinner")
    bibibi.splitExpensesCalculation()

    assert bibibi.ResultsDB == {toan: {bich:25.0,toan:0},                                
                                bich: {toan:0,bich:0},                               
                               }
    ...
    bibibi.splitExpensesCalculation()
    assert bibibi.ResultsDB == {toan: {bich:0,toan:0,eugene:0},                                
                                bich: {toan:35.0,bich:0,eugene:60.0},                               
                                eugene:{toan:0,bich:0,eugene:0}
                                } 

    bibibi.addExpenseSinglePayer([toan,eugene],220,eugene,"dinner")
    
    bibibi.splitExpensesCalculation()
    assert bibibi.ResultsDB == {toan: {bich:0,toan:0,eugene:0},                                
                                bich: {toan:35.0,bich:0,eugene:60.0},                               
                                eugene:{toan:110.0,bich:0,eugene:0}
                                } 